...
# so the invariant set adapts when the schema changes without requiring
# edits to this file.
# ---------------------------------------------------------------------------
...
PER_ATTRIBUTE_INVARIANTS: list[tuple[str, Any]] = [
    ("Inv1", inv1_not_proxy),
...
#
# Evaluated in order after per-attribute pruning has stabilised.
...
def inv3_monotonic_income_score(obj: dict, sl_spec: dict) -> tuple[bool, str]:
    ...
    Monotonic(income → score). See decidability note in module docstring.
    """
...
GLOBAL_INVARIANTS: list[tuple[str, Any]] = [
