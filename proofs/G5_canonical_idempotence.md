...
These three axioms together assert that every stage of the canonical pipeline is a fixed point on canonical forms.
...
Lemma 2 — ε leaves canonical forms unchanged.
...
Apply the fixed‑point axioms step by step.
...
Step 2 — Apply Axiom 2 (ε fixed point)
\varepsilon (\mathrm{Canon}(x))=\mathrm{Canon}(x).
Thus:
...
- ε does not modify canonical forms.
