...
If \mathrm{Safe}(x), then \mathrm{Collapse}(x)=\mathrm{Canon}(x).
...
Lemma 2 — Collapse writes Canon(x) whenever it writes anything.
Collapse writes to the trace only when \mathrm{Safe}(x) holds.
In that case:
\mathrm{Collapse}(x)=\mathrm{Canon}(x).
Thus any trace entry written by Collapse contains \mathrm{Canon}(x) as its committed representation.
□
...
- Canonical forms always being safe (G3).
